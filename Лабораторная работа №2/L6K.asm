...
set_prmaus proc 
		...
		mov ax,0ch
		mov cx,10b	; переривання при натиснутій правій кнопці
		mov bx, seg prmaus
		mov es, bx
		lea dx,prmaus
		int 33h
...
ret_prmaus endp
	...
	;Власна процедура обробки переривань
			;від мишки
prmaus proc far
	...
	push 0b800h
	pop es
	push _data	;збереження вмісту регістрів 
	pop ds		;що використовуються у процедурі
		
	shr dx, 3	;приведення координат переданих мишкою		
	shr cx, 3	;до виду 80, 25
	...
	mov bx, dx
	imul bx, 160
	add bx, cx	;визначення позиції в відео-памяті
	add bx, cx	;точки на яку натиснуто
...
;SETINT
	...
	call 		set_prmaus	;перехоплення переривань від мишки
